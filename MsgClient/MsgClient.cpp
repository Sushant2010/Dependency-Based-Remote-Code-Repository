/////////////////////////////////////////////////////////////////////
//  MsgClient.cpp - Sending files to server from the Client        //
//  ver 1.0                                                        //
...
void MsgClient::startSender()
{
	try
	{
		...
		while (true)
		{
			HttpMessage request = sendQueue.deQ();
...
void MsgClient::processRequest(SocketConnecter& si, std::string request, std::string body)
{
	try {   
		...
		{   size_t pos = 0;
			std::string bod = body; std::vector<std::string> files; 
			...
			HttpMessage hMsg; hMsg = makeMessage(1, "quit", "toAddr:localhost:8080");
			sendMessage(hMsg, si);
			Show::write("\n  All done folks");}
...
{
	HttpMessage result = receiveQueue.deQ();
	std::string files = result.findValue("getList");
	return files;
}
//----< this defines processing to frame messages >------------------

HttpMessage HandleClient::readMessage(Socket& socket)
{
	connClosed_ = false;
	HttpMessage msg;
	while (true) {
...
*/
bool HandleClient::readFile(const std::string& filename, size_t fileSize, Socket& socket)
{
...
void HandleClient::operator()(Socket socket)
{
	//std::cout << "\n\n in Client operator() \n\n";
	while (true)
	{
		HttpMessage msg = readMessage(socket);
		if (connClosed_ || msg.bodyString() == "quit")
		{
...
int main()
{
  ::SetConsoleTitle(L"Running Clients");
